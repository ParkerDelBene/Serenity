...
class AudioOutput {
  ...
  */
  ...
  void addStream(Stream<dynamic> audioStream) {
    audioStream.listen((data) {
      if (Platform.isLinux) {
        ...
        windowsNumFrameAvailable[0] =
            windowsBufferFrameSize[0] - windowsNumFramePadding[0];
  ...
  Future<void> initWindowsAudioProcess() async {
    ...
    pEnumerator = IMMDeviceEnumerator(COMObject.createFromID(
    ...
    hr = audioDevice.activate(
        GUIDFromString(IID_IAudioClient), CLSCTX_ALL, nullptr, pAudioClient);
    ...
    hr = audioClient.initialize(
        AUDCLNT_SHAREMODE_SHARED, 0, REFTIMES_PER_SEC, 0, pwfx[0], nullptr);
    ...
    hr = audioClient.getService(
        GUIDFromString(IID_IAudioRenderClient), pRenderClient);
    ...
    hr = renderClient.getBuffer(
        windowsBufferFrameSize[0].toInt(), windowsSoundData);
